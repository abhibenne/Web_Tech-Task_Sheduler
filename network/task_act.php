...
$value = isset($_POST['myKey']) ? $_POST['myKey'] : null;
if($value)
{
	print_r($_POST);
print_r($_POST['myKey']);
...
if(isset($_SESSION['user_name']))
{
	...
	if(!$run_user)
	{
	$insert = "insert into tasker (user_id, task,date,updated,status) values('$user_id', '$all',NOW(),'yes','$status')";
	$run = mysqli_query($con, $insert);
	if($run)
	{
		echo "Inserted Properly<br>";
